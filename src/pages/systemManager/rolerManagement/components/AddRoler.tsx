...
import { tree2Array } from '@/utils/utilsJS';
...
const AddPerson: React.FC<PropsInfo> = (props: any) => {
  ...
  const add = (params: any) => {
    ...
    const ids = update.power.map((item: any) => {
      return item.value;
    });
    const data = { ...params };
    data.powerIdList = ids;
    data.power = ids;
    saveRolerInfo(data)
      .then((res) => {
        if (res.code === 200) {
            ...
            >
             ...
             fieldNames={{ label: 'title', value: 'key', children: 'children' }}
                  ...
                  onChange={(value: any, node: any, extra: any) => {
                    ...
                    const treeArr = tree2Array(powerTree, extra.triggerValue, 'key');
                    const checked = value.filter((ite: any) => {
                      return ite.value === extra.triggerValue;
                    });
                    if (checked.length === 0) {
                      const arr: any[] = [];
                      const treeKeys = treeArr.map((a: any) => {
                        return a.key;
                      });
                      value.forEach((item: any) => {
                        if (!treeKeys.includes(item.value)) {
                          arr.push(item);
                        }
                      });
                      newArr = arr;
                    ...
                    form.setFieldsValue({ power: newArr });
                  }}
                />
              )}
            </Form.Item>
