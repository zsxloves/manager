...
import { Phone } from '@/utils/utilsJS';

interface Props {
  show: boolean;
  id?: string;
  onConfirm: (page: number, size: number) => any;
  onCancel: any;
  item: any;
  current: number;
  pageSize: number;
...
const AddVenue: React.FC<Props> = (props) => {
  ...
  const { show, onCancel, onConfirm, item, current, pageSize } = props;
      ...
      <Form form={form} style={{ paddingRight: 60 }} labelCol={{ style: { width: 140 } }}>
        ...
        <Row>
          ...
          <Col span={11} offset={2}>
            ...
            >
              <Input maxLength={11} placeholder="请输入" autoComplete="off" />
            </Form.Item>
          </Col>
        </Row>
        <Row>
